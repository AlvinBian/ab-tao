...
const healthColor = computed(() => {
  if (healthPct.value >= 80)
    return successColor.value
  if (healthPct.value >= 50)
      ...
      <el-col :span="5">
        <el-card shadow="never" style="height:148px">
          <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%">
            ...
            <div style="margin-top:6px; font-size:13px; color:var(--el-text-color-secondary)">
              健康度
            </div>
          </div>
        </el-card>
    ...
    <el-row :gutter="16" style="margin-bottom:16px">
      ...
      <el-col :span="12">
        <el-card shadow="never" style="height:100%">
          <template #header>
            <span>Top Projects（Sessions 數）</span>
          </template>
      ...
      <el-col :span="12">
        ...
        <el-card shadow="never">
          ...
          <el-descriptions :column="1" border size="small">
            <el-descriptions-item label="Cache">
              {{ formatBytes(d?.diskUsage?.cache ?? 0) }}
            </el-descriptions-item>
            <el-descriptions-item label="Dist">
              {{ formatBytes(d?.diskUsage?.dist ?? 0) }}
            </el-descriptions-item>
            <el-descriptions-item label="Projects">
              {{ formatBytes(d?.diskUsage?.claudeProjects ?? 0) }}
            </el-descriptions-item>
          </el-descriptions>
        </el-card>
      </el-col>
