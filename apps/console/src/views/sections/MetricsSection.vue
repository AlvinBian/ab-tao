      ...
      <div class="metrics-tab">
        <div class="tab-header">
          ...
          <div class="connection-status">
            ...
            <el-button size="small" plain @click="connectMetrics">重新連線</el-button>
          </div>
        </div>
        <el-alert
          v-if="metricsError"
        ...
        <el-table :data="metricsRows" stripe style="width: 100%" max-height="480">
          <el-table-column prop="timestamp" label="Timestamp" min-width="200">
            <template #default="{ row }">
              <el-text size="small" type="info">{{ row.timestamp }}</el-text>
            </template>
          </el-table-column>
          <el-table-column prop="event_type" label="Event Type" min-width="160">
            <template #default="{ row }">
              <el-tag type="primary" size="small" effect="plain">{{ row.event_type }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="value" label="Value" min-width="320">
            <template #default="{ row }">
              <el-text size="small" truncated>{{ row.value }}</el-text>
            </template>
          </el-table-column>
        </el-table>
        <div v-if="metricsRows.length === 0 && !metricsError" class="empty-hint">
          <el-text type="secondary" size="small">尚無指標資料，等待 metrics.jsonl 寫入…</el-text>
        </div>
      </div>
    </template>
      ...
      <div class="metrics-tab">
        ...
        <el-table :data="triggerRows" stripe style="width: 100%">
          ...
          <el-table-column prop="trigger" label="觸發條件" min-width="240">
            <template #default="{ row }">
              <el-text size="small">{{ row.trigger }}</el-text>
            </template>
          </el-table-column>
          <el-table-column prop="currentStatus" label="當前狀態" min-width="120">
            <template #default="{ row }">
              <el-tag type="warning" size="small" effect="plain">{{ row.currentStatus }}</el-tag>
            </template>
          </el-table-column>
        </el-table>
      </div>
      ...
      <div class="metrics-tab">
        <div class="tab-header">
          ...
          <div class="connection-status">
            ...
            <el-button size="small" plain @click="connectFailurePatterns">重新連線</el-button>
          </div>
        </div>
        <el-alert
