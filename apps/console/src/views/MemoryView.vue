...
<template>
  <div v-loading="store.loading">
    ...
    <el-card shadow="never" style="margin-bottom:16px">
      <el-row :gutter="16" align="middle">
        <el-col :span="6">
          ...
          <div style="font-size:11px; color:var(--el-text-color-placeholder); margin-top:2px">
            全域 {{ global?.memory?.length ?? 0 }} + 專案 {{ totalMemory - (global?.memory?.length ?? 0) }}
          </div>
        </el-col>
        <el-col :span="6">
          <el-statistic title="Plans（全部）" :value="totalPlans" />
          <div style="font-size:11px; color:var(--el-text-color-placeholder); margin-top:2px">
            全域 {{ global?.plans?.length ?? 0 }} + 專案 {{ totalPlans - (global?.plans?.length ?? 0) }}
          </div>
        </el-col>
        <el-col :span="6">
          <el-statistic title="Tasks（全部）" :value="totalTasks" />
          <div style="font-size:11px; color:var(--el-text-color-placeholder); margin-top:2px">
            全域 {{ global?.tasks?.length ?? 0 }} + 專案 {{ totalTasks - (global?.tasks?.length ?? 0) }}
          </div>
        </el-col>
        <el-col :span="6">
    ...
    <el-card v-if="hasMemoryData" shadow="never" style="margin-bottom:16px">
      <template #header>
        <span>Memory 分佈（Global + 各專案）</span>
      </template>
    ...
    <el-card v-if="projects.length > 0" shadow="never" style="margin-bottom:16px">
      <template #header>
        <span>專案 Memory 檔案數排行（Top 10）</span>
      </template>
      <MemorySizeBar
    ...
    <el-card shadow="never">
      <template #header>
        <span>專案 Memory（{{ projects.length }} 個專案）</span>
      </template>
        ...
        >
          <template #title>
            ...
            <el-tag size="small" style="margin-left:8px">
              {{ proj.memory.length + proj.plans.length + proj.tasks.length }} 個檔案
            </el-tag>
          </template>
          <el-row :gutter="12">
            <el-col :span="8">
              ...
              <el-tag v-for="f in proj.memory" :key="f" size="small" style="margin:2px" type="primary">
                {{ f }}
              </el-tag>
            ...
            <el-col :span="8">
              ...
              <el-tag v-for="f in proj.plans" :key="f" size="small" style="margin:2px" type="success">
                {{ f }}
              </el-tag>
            ...
            <el-col :span="8">
              ...
              <el-tag v-for="f in proj.tasks" :key="f" size="small" style="margin:2px" type="warning">
                {{ f }}
              </el-tag>
