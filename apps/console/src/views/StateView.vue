...
<template>
  <div v-loading="store.loading">
    ...
    <el-card shadow="never" style="margin-bottom:16px">
      <template #header><span>State 設定</span></template>
      <el-descriptions :column="3" border size="small">
        <el-descriptions-item label="版本">{{ d?.version ?? "—" }}</el-descriptions-item>
        <el-descriptions-item label="Sync 工具">{{ d?.sync?.tool ?? "—" }}</el-descriptions-item>
      ...
      <el-table v-if="drift.length > 0" :data="drift" stripe size="small">
        ...
        <el-table-column label="狀態" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="driftTagType(row.decision)" size="small">{{ row.decision }}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-card shadow="never">
      <template #header><span>安裝選擇記錄（{{ choiceEntries.length }}）</span></template>
      <el-table :data="choiceEntries" stripe size="small" max-height="300">
        <el-table-column prop="path" label="路徑" min-width="200" show-overflow-tooltip />
        <el-table-column prop="decision" label="決策" width="100">
          <template #default="{ row }">
            <el-tag size="small" :type="row.decision === 'skip' ? 'info' : 'primary'">{{ row.decision }}</el-tag>
          </template>
        </el-table-column>
