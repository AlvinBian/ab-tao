...
export async function settingsRouter(req, res, url, json) {
  ...
  if (req.method === 'PUT' && url.pathname === '/api/settings') {
    if (rejectUntrusted(req, res, json))
      return true
    ...
    await withLock(res, json, async () => {
      if (await backupOrFail(P, res, json))
        return
  ...
  if (req.method === 'PATCH' && hookPatchMatch) {
    ...
    const idx = Number.parseInt(hookPatchMatch[2], 10)
    ...
    await withLock(res, json, async () => {
      ...
      if (enabled === false) {
        ...
        if (!settings._abTao)
          settings._abTao = {}
        if (!settings._abTao.disabledHooks)
          settings._abTao.disabledHooks = {}
  ...
  if (req.method === 'DELETE' && hookDeleteMatch) {
    ...
    const idx = Number.parseInt(hookDeleteMatch[2], 10)
    if (Number.isNaN(idx) || idx < 0) {
      json(res, 400, 'idx 必須為非負整數', { code: 'INVALID_IDX' }, 400)
      return true
    }
