...
test("getPath：不存在回傳 undefined", () => {
	assert.equal(getPath({ a: 1 }, "b.c"), undefined);
});

// ── setPath ────────────────────────────────────────────────────────

test("setPath：設定巢狀值（自動建立中間物件）", () => {
	const obj = {};
...
test("mergeArray local-wins：本地陣列完全勝出", () => {
	const result = mergeArray(["a", "b"], ["c", "d"], "local-wins");
	assert.deepEqual(result, ["c", "d"]);
});

test("mergeArray：local 非陣列時 fallback 到 template", () => {
...
test("mergeConfig：union array 合併 + dedupe", () => {
	const template = {
...
test("mergeConfig：local-wins array", () => {
	const template = {
...
test("mergeConfig：template key local 未設定 → 採用 template 預設", () => {
...
test("mergeConfig：不修改原始輸入物件（純函數）", () => {
	const template = { a: 1 };
	const local = { b: 2 };
