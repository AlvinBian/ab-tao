...
describe('CODING_STYLE_CONTENT 內容驗證', () => {
  ...
  it('包含不可變性相關內容', () => {
    assert.ok(
      CODING_STYLE_CONTENT.includes('immutable') || CODING_STYLE_CONTENT.includes('不可變'),
    );
  });
  ...
  it('包含 async/await 相關內容', () => {
    assert.ok(CODING_STYLE_CONTENT.includes('async') || CODING_STYLE_CONTENT.includes('await'));
  });
});
...
describe('TESTING_CONTENT 內容驗證', () => {
  ...
  it('包含 src/ 和 tests/ 相關內容', () => {
    assert.ok(TESTING_CONTENT.includes('src/') || TESTING_CONTENT.includes('tests/'));
  });
  ...
  it('包含行為驅動命名相關內容', () => {
    assert.ok(TESTING_CONTENT.includes('行為') || TESTING_CONTENT.includes('命名'));
  });
...
describe('GIT_WORKFLOW_CONTENT 內容驗證', () => {
  ...
  it('包含 push -u 旗標相關內容', () => {
    assert.ok(GIT_WORKFLOW_CONTENT.includes('-u') || GIT_WORKFLOW_CONTENT.includes('push'));
  });
});
