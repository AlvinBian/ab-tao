...
test("buildSyncPlan：FORBIDDEN_DIRS 內的項目回傳 forbiddenSkip", async () => {
...
test("buildSyncPlan：FORBIDDEN_DIRS 項目不產生 create 或 overwriteFile", async () => {
	const templateRoot = createMockTemplate();
	const homeRoot = mkdtempSync(join(tmpdir(), "abtao-home-"));
...
test("buildSyncPlan：非 forbidden 的普通檔案（home 不存在）應為 create", async () => {
	const templateRoot = createMockTemplate();
	const homeRoot = mkdtempSync(join(tmpdir(), "abtao-home-"));
...
test("buildSyncPlan：ADDITIVE_DIRS 中 drift 的檔案為 additiveKeep", async () => {
	const templateRoot = createMockTemplate();
	const homeRoot = mkdtempSync(join(tmpdir(), "abtao-home-"));
