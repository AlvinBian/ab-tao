...
describe("applyMemoryUpdate（MEMORY.md 邏輯）", () => {
	test("首次建立：生成 manual markers + auto section", () => {
	...
	test("無 markers 的舊 MEMORY.md：包裹現有內容，不丟失", () => {
	...
	test("manual section 保留，auto section 更新", () => {
		const existing = [
			MANUAL_START,
	...
	test("空 memory dir 時 auto section 只有 markers", () => {
		const result = applyMemoryUpdate(null, []);
		assert.ok(result.includes(AUTO_START));
