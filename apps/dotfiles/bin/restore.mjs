...
async function main() {
  ...
  const hasOriginal
    = fs.existsSync(ORIGINAL_DIR) || fs.existsSync(LEGACY_ORIGINAL_DIR)
  const actualOriginalDir = fs.existsSync(ORIGINAL_DIR)
    ? ORIGINAL_DIR
    : LEGACY_ORIGINAL_DIR
  ...
  if (selected === '__original__') {
    ...
    p.log.info(
      `即將從 ${pc.cyan('~/.ab-tao-backup-original/')} 恢復：\n  ${items.join(', ')}`,
    )
    ...
    for (const item of items) {
      ...
      if (!KNOWN_RESTORE_ENTRIES.has(item))
        continue
