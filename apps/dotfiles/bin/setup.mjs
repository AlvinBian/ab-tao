...
async function main() {
  ...
  if (prev && !flagAll && !flagQuick) {
    ...
    if (action === 'status') {
      ...
      if (adjustMap[adjustAction])
        await adjustMap[adjustAction]()
      p.outro('調整完成')
      process.exit(0)
    }
  ...
  const fontHint = preferences ? getNerdFontHint(preferences) : null
  if (fontHint)
    p.log.warn(fontHint)
