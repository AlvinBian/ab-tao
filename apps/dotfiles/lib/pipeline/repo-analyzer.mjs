...
export function buildRepoSummary(repoName, analysis) {
  ...
  const { classified, unclassified } = classifyBatch([...npmDeps, ...npmDevDeps], phpDepNames);
  if (classified.size > 0) {
    const preParts = [...classified.entries()].map(
      ([cat, pkgs]) => `${cat}: ${pkgs.slice(0, 3).join(', ')}${pkgs.length > 3 ? ' …' : ''}`,
    );
  ...
  return {
    ...
    meta,
    npmDeps: new Set(
      [...Object.keys(deps), ...Object.keys(devDeps)].filter((n) => !n.startsWith('@types/')),
    ),
    allDeps: { deps, devDeps, phpDeps },
