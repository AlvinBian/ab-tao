...
export function inferNpmCategory(keywords, desc) {
  ...
  if (kw.has('test') || kw.has('testing') || /test(ing)?\s+(framework|runner|library)/.test(desc))
    return 'testing';
  if (kw.has('state-management') || kw.has('store') || /state management/.test(desc))
    return 'state';
  if (
  ...
  if (kw.has('websocket') || kw.has('realtime') || kw.has('socket') || kw.has('real-time'))
    return 'realtime';
  if (
    kw.has('auth') ||
  ...
  if (
    ...
    return 'validation';
  if (kw.has('bundler') || kw.has('build-tool') || /\bbundler\b|build tool/.test(desc))
    return 'build';
  ...
  if (
    ...
    return 'date';
  if (kw.has('analytics') || kw.has('tracking') || kw.has('monitoring') || kw.has('observability'))
    return 'analytics';
  return 'library';
}
