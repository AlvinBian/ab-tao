...
export function warmupCli() {
  ...
  const child = spawn(
    cli,
    ['-p', 'hi', '--model', 'haiku', '--effort', 'low', ...BASE_FLAGS, '--output-format', 'json'],
...
export async function callClaude(
  prompt,
  { timeoutMs = AI_TIMEOUT, model = AI_MODEL, effort = AI_EFFORT, retries = 1 } = {},
) {
  const cli = findCli();
  ...
  const args = ['-p', prompt, '--model', model, '--effort', effort, ...BASE_FLAGS];
...
export function callClaudeJSONStream(
  prompt,
  { timeoutMs = AI_TIMEOUT, model = AI_MODEL, effort = AI_EFFORT, onProgress = () => {} } = {},
) {
