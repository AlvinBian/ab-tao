                          ...
                          },
                          {
                            ...
                            task: async (_, sub) => {
                              ...
                              if (fs.existsSync(repoEnv)) {
                                ...
                                const channel =
                                  repoEnvContent.match(/^SLACK_NOTIFY_CHANNEL=(.*)$/m)?.[1] ?? '';
                                const mode =
                                  repoEnvContent.match(/^SLACK_NOTIFY_MODE=(.*)$/m)?.[1] ?? '';
                                const channelName =
                                  repoEnvContent.match(/^SLACK_NOTIFY_CHANNEL_NAME=(.*)$/m)?.[1] ??
                                  '';
                                const userId =
                                  repoEnvContent.match(/^SLACK_NOTIFY_USER_ID=(.*)$/m)?.[1] ?? '';
                                if (channel) {
                ...
                task.newListr(
                  [
                    // [3] AI 外部資源 + 技術棧 Stacks
                    {
                      ...
                      enabled: () =>
                        has('ecc') &&
                        ((plan.ecc?.length ?? 0) > 0 || (plan.techStacks?.length ?? 0) > 0),
                      task: (_, subtask) =>
                        subtask.newListr(
                          [
                            {
                              ...
                              task: async (_, sub) => {
                                if (
                                  ...
                                  try {
                                    ...
                                    shared.syncResult = buildSyncResult(fetchedSources, eccByType);
                                    ...
                                    const added = shared.syncResult.downloaded?.length || 0;
                    ...
                    // [4] CLAUDE.md 生成 → ~/.claude/projects/
                    {
                      ...
                      task: async (_, subtask) => {
                        ...
                        if (result.claudeignoreCount > 0)
                          parts.push(`.claudeignore：${result.claudeignoreCount} 個 repo`);
