# Git / PR 慣例（commit、發 PR、開分支時 Read）

> 原 `rules/git-and-pr.md` 的動作語義內容（2026-07 遷出：rules paths 只能按檔案路徑觸發，
> 「commit 前」「發 PR 時」這類動作時機由 `claude-md/05-security.md` 的指針引導 Read 本檔）。
> 紅線本體（三豁免 / merge 硬禁 / force push backup）常駐 05；本檔為慣例細節。
...
## Stack 強制規則（違反即破壞 stack）

❌ 禁止 `gh pr merge` / GitHub auto-merge（見 05 紅線）
❌ 禁止在 stack 中段直接 `git rebase main`（必走 `git-spice repo sync`）
❌ 禁止 force push 前未建 `backup/<branch>`

✅ PR merge 唯一方式：GitHub UI 手動點擊（PR-N merge 後才能 merge PR-N+1）
