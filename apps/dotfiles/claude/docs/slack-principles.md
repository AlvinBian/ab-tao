...
**MCP 工具雙軌行為（2026-07-15/16 實測，與本節手寫 mrkdwn 規則不同）**：**預設軌＝MCP 直發**（凡由 Claude 代發皆經轉換器、用 standard markdown；本節上方 mrkdwn 紅線僅適用於手貼文本與 Slack API 直呼）。`slack_send_message`（直發）內建 standard markdown → mrkdwn 轉換器——寫 `**雙星號**` 得粗體、`[文字](url)` 得連結；⚠️ 寫 `*單星號*` 會被轉成斜體。已知 bug：`**…**` 緊鄰全形標點會解析失敗輸出字面 `**`，且**觸發不穩定**（實測 `｜` 與 `（` 均中招、同訊息另一處 `**…**（` 卻正常）→ 一律視為會壞：**`**` 前後只放半形字元**，或一行只放一組粗體；發送後必用 `slack_read_thread` 回讀驗證。`slack_send_message_draft`（編輯框草稿）為 WYSIWYG **純文字**，任何標記皆字面顯示且送出原樣 → 草稿禁用一切標記符號，連結用裸 URL 獨立一行；此軌預設不用，僅使用者明確要求「放編輯框」時採用。
...
- PR Review 結果回覆：**單行** `#PR號 ✅ LGTM 👍` / `#PR號 💬 N findings` ＋ PR 連結（細節全在 PR 評論；僅 P0/P1 各追加 1 行；**不套四層分段**）
