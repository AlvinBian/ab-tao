...
- [ ] `claude-md/` 12 個 section 檔全部存在（00–05, 07–08, 11–13, 15）
- [ ] `rules/` 9 個條件規則檔有正確 `paths:` frontmatter（api-and-data / vue-nuxt / typescript / testing / migrations / barrel-exports / git-and-pr / code-quality / settings-edit）
- [ ] `rules/barrel-exports.md` 含 7 條 sub-rule（統一導出 3 + 統一引入 2 + 適用範圍 1 + 遷移策略 1）
- [ ] `docs/` 12 個參考文件存在（rtk / audit-checklists / config-map / slack-principles / slack-audience-profiles / ai-dispatcher / federated-memory / self-evolution / local-tools / state-system-details / agent-dag-parallel / self-correction-details）
