...
import { parseSource } from '../core/source-classifier.mjs'
import {
  ALL_AGENTS,
  ALL_COMMANDS,
  ALL_RULES,
...
export function detectLegacyInstallation() {
  ...
  // 找出非 ab-tao 管理的配置，進一步按 frontmatter 分類為 AI source vs 真正自訂
  const notManagedCommands = existingCommands.filter(c => !ALL_MANAGED.commands.has(c))
  const notManagedAgents = existingAgents.filter(a => !ALL_MANAGED.agents.has(a))
  const notManagedRules = existingRules.filter(r => !ALL_MANAGED.rules.has(r))

  function splitAiVsCustom(names, dir) {
    const aiSource = []
    const custom = []
    for (const name of names) {
      const src = parseSource(path.join(dir, `${name}.md`))
      if (src !== 'custom') {
        aiSource.push({ name, source: src })
      }
      else {
        custom.push(name)
      }
    }
    return { aiSource, custom }
  }

  const { aiSource: aiSourceCommands, custom: userCustomCommands } = splitAiVsCustom(notManagedCommands, commandsDir)
  const { aiSource: aiSourceAgents, custom: userCustomAgents } = splitAiVsCustom(notManagedAgents, agentsDir)
  const userCustomRules = notManagedRules // rules 暫不分類（rules 沒有 AI source 來源）

  const totalProjectItems
    = projectCommandsInGlobal.length
      + projectAgentsInGlobal.length
      + projectRulesInGlobal.length
  const totalAiSourceItems = aiSourceCommands.length + aiSourceAgents.length
  const needsUpgrade
    = totalProjectItems > 0
      || hasOldKkday
      || settingsNeedsUpgrade
      || hooksNeedsUpgrade
      || totalAiSourceItems > 0
...
export async function runUpgrade(legacyInfo) {
  ...
  // AI source 部署檔（ecc / anthropic 等，可選清除）
  const aiCount = (legacyInfo.aiSourceCommands?.length ?? 0) + (legacyInfo.aiSourceAgents?.length ?? 0)
  if (aiCount > 0) {
    // 按 source 分組統計
    const sourceCounts = {}
    for (const { source } of [...(legacyInfo.aiSourceCommands ?? []), ...(legacyInfo.aiSourceAgents ?? [])]) {
      sourceCounts[source] = (sourceCounts[source] ?? 0) + 1
    }
    const sourceTag = Object.entries(sourceCounts).map(([s, n]) => `${s}: ${n}`).join(', ')
    lines.push('')
    lines.push(`${idx}. 🤖 ${aiCount} 個 AI source 部署檔（${sourceTag}）`)
    let ci = 1
    if (legacyInfo.aiSourceCommands?.length) {
      lines.push(`   ${idx}.${ci} Commands（${legacyInfo.aiSourceCommands.length}）`)
      legacyInfo.aiSourceCommands.forEach(({ name }) => {
        lines.push(descBullet(name, 'commands', cd))
      })
      ci++
    }
    if (legacyInfo.aiSourceAgents?.length) {
      lines.push(`   ${idx}.${ci} Agents（${legacyInfo.aiSourceAgents.length}）`)
      legacyInfo.aiSourceAgents.forEach(({ name }) => {
        lines.push(descBullet(name, 'agents', cd))
      })
    }
    idx++
  }
  ...
  if (customCount > 0) {
    lines.push('')
    lines.push(`${idx}. ⚠ ${customCount} 個手動自訂配置（不會刪除）`)
    let ci = 1
  ...
  const options = [
    {
      value: 'upgrade',
      label: '✨ 清除殘留配置（推薦） — 備份 → 清理舊配置 → 更新 settings/hooks',
    },
  ...
  if (aiCount > 0) {
    options.splice(1, 0, {
      value: 'clean-ai',
      label: `🧹 清除 AI source 部署檔（${aiCount} 個）— 保留 ab-tao 核心 + 手動自訂`,
    })
  }
  ...
  if (action === 'clean-ai') {
    await doCleanAiSources(legacyInfo)
    return 'cleaned-ai'
  }
...
/**
 * 清除 AI source（ecc / anthropic 等）部署到 ~/.claude/ 的 commands / agents。
 * 保留 ab-tao 核心配置與手動自訂配置不動。
 *
 * @param {object} legacyInfo - detectLegacyInstallation 返回的偵測結果
 * @returns {Promise<void>}
 */
async function doCleanAiSources(legacyInfo) {
  const commandsDir = path.join(CLAUDE_DIR, 'commands')
  const agentsDir = path.join(CLAUDE_DIR, 'agents')

  p.log.info('🗂️ 備份 AI source 配置...')
  await Promise.all([
    backupIfExists(commandsDir, 'clean-ai/commands'),
    backupIfExists(agentsDir, 'clean-ai/agents'),
  ])

  let removed = 0
  for (const { name } of (legacyInfo.aiSourceCommands ?? [])) {
  ...
  for (const { name } of (legacyInfo.aiSourceAgents ?? [])) {
  ...
  p.log.success(`已清除 ${removed} 個 AI source 部署檔（備份在 dist/backup/clean-ai/）`)
}
...
async function doClean() {
  ...
  if (fs.existsSync(settingsPath)) {
    try {
      ...
      if (settings.hooks)
        savedUserSettings.hooks = settings.hooks
