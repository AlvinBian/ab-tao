...
function cleanEnv() {
  const { ANTHROPIC_API_KEY: _ANTHROPIC_API_KEY, ...rest } = process.env
  return rest
}
...
export function warmupCli() {
  ...
  child.on('error', (e) => {
    ...
    if (process.env.DEBUG)
      process.stderr.write(`[warmupCli] ${e.message}\n`)
  })
 ...
 * @param {string} [options.model] - 模型別名（haiku/sonnet/opus）
 * @param {string} [options.effort] - 推理強度（low/medium/high/max）
 * @param {number} [options.retries] - 重試次數
 * @param {boolean} [options.silent] - 是否靜默失敗（不輸出錯誤訊息）
...
export async function callClaude(
  ...
  if (!cli)
    throw new Error('claude CLI 未安裝，請執行 claude install')
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (text && !text.includes('Not logged in'))
        return text
    }
    catch (e) {
      ...
      if (out && !out.includes('Not logged in'))
        return out
      // 最後一次重試也失敗
      if (attempt >= retries) {
        if (!silent)
...
function extractJSON(text) {
  ...
  // 2. 去掉 ```json ... ``` 包裹
  // eslint-disable-next-line regexp/no-super-linear-backtracking -- code block pattern with intentional lazy matching
