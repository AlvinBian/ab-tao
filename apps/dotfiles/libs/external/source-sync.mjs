...
function getCacheDir(cacheBase, sourceName) {
  return path.join(cacheBase, sourceName.replace(/[^\w-]/g, '_'))
}
...
async function fetchIndex(source, techStacks) {
  ...
  for (const r of results) {
    ...
    const v = r.value
    if (v.type === 'commands')
      index.commands.push(...v.files)
    else if (v.type === 'agents')
      index.agents.push(...v.files)
    else if (v.type === 'rules')
      index.rules.push(...v.files)
  }
  return index
}
  ...
  function isLangMatch(name) {
    for (const pf of LANG_PREFIXES) {
      if (name.startsWith(pf))
        return matchedLangs.has(pf.replace(/-$/, ''))
    }
    return true
  }
  return {
...
/**
 * 將 marker 注入到 frontmatter 之後（而非整個檔案最前面）
 * 確保 Claude Code 能正確解析 frontmatter 中的 description: 欄位
 */
function injectMarker(marker, content) {
  if (content.startsWith('---\n')) {
    const closingIdx = content.indexOf('\n---\n', 4)
    if (closingIdx !== -1) {
      return (
        content.slice(0, closingIdx + 5)
        + marker
        + content.slice(closingIdx + 5)
      )
    }
  }
...
/**
 * 判斷檔案是否為用戶自訂（存在且不含任何 ab-tao 生成 marker）
 */
function isUserCustomized(filePath) {
  if (!fs.existsSync(filePath))
    return false
  const content = fs.readFileSync(filePath, 'utf8')
  // 使用 includes 而非 startsWith：marker 現在插在 frontmatter 之後
  // 向下相容：同時檢查舊 marker（startsWith）
  return (
    !content.includes(GENERATED_MARKER)
    && !content.startsWith(LEGACY_MARKER)
    && !/<!-- generated by ab-tao::\S+ -->/.test(content)
  )
}
...
function mergeHooksJson(destPath, incomingContent) {
  ...
  if (fs.existsSync(destPath)) {
    try {
      if (fs.readFileSync(destPath, 'utf8') === resultJson)
        return
    }
    catch {
      /* proceed with write */
...
export async function writeSkillFiles(skillSources, targetDir, opts = {}) {
  const skipped = []
  const written = []
  const failed = []
  const skillsRoot = path.join(targetDir, 'skills')
  const total = skillSources.reduce(
    (s, src) => s + (src.skills?.length || 0),
    0,
  )
  let current = 0
  ...
  for (const src of skillSources) {
    if (!src.skills?.length)
      continue
    const safeSource = path.basename(src.source)
    ...
    for (const skill of src.skills) {
      ...
      if (!skill.content?.trim()) {
        skipped.push(`skills/${safeSource}/${safeName} (空內容)`)
        opts.onProgress?.({
          current: ++current,
          total,
          name: safeName,
      ...
      // 來源衝突：檔案已存在且由不同來源生成 → 跳過
      if (fs.existsSync(skillPath)) {
        try {
          const head = fs.readFileSync(skillPath, 'utf8').slice(0, 80)
          const existingMarkerMatch = head.match(
            /<!-- generated by ab-tao::(\S+) -->/,
          )
          if (existingMarkerMatch && existingMarkerMatch[1] !== safeSource) {
            skipped.push(
              `skills/${safeName} (來源衝突：${existingMarkerMatch[1]} 優先)`,
            )
            opts.onProgress?.({
              current: ++current,
              total,
      ...
      try {
        ...
        const sourceMarker = `<!-- generated by ab-tao::${safeSource} -->\n`
        fs.writeFileSync(skillPath, injectMarker(sourceMarker, routed), 'utf8')
        written.push(skillPath)
      }
      catch (err) {
        failed.push({ name: safeName, source: safeSource, error: err.message })
      }
