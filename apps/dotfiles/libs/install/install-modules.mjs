 ...
 * @param {object | null} [session] - 上次 session（用於預選模組）
...
export async function handleInstallModules(
  ...
  const def = Object.values(selectable)[0]
  const key = Object.keys(selectable)[0]
  if (!def || !key) {
    logger?.warn(`${stepLabel}step.selectable 為空，跳過`)
    return { modules: [] }
  }
