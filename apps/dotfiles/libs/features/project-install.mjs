...
export default {
  ...
  async configure(ctx) {
    ...
    if (reposDep?.repos) {
      for (const r of reposDep.repos) {
        if (r.localPath)
          localPaths[r.fullName] = r.localPath
      }
    }
      ...
      for (const type of ['commands', 'agents', 'rules', 'skills']) {
        ...
        for (const item of srcItems) {
          ...
          if (type === 'skills') {
            ...
            if (winner && winner.source !== src.name)
              continue
          }
  ...
  complete(results) {
    ...
    const counts = []
    if (sel.commands?.length)
      counts.push(`${sel.commands.length} commands`)
    if (sel.agents?.length)
      counts.push(`${sel.agents.length} agents`)
    if (sel.rules?.length)
      counts.push(`${sel.rules.length} rules`)
    if (sel.skills?.length)
      counts.push(`${sel.skills.length} skills`)
    if (counts.length) {
  ...
  report(results) {
    if (!results)
      return { feature: 'project-install' }
    const sel = results.installSelections || {}
    return {
