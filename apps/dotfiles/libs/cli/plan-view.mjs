...
export function formatExistingState(existing) {
  ...
  if (existTotal > 0) {
    ...
    if (existing.commands.length)
      parts.push(`${existing.commands.length} cmd`)
    if (existing.agents.length)
      parts.push(`${existing.agents.length} agent`)
    if (existing.rules.length)
      parts.push(`${existing.rules.length} rule`)
    const extras = []
    if (existing.hasSettings)
      extras.push('settings')
    if (existing.hasHooks)
      extras.push('hooks')
  ...
  for (const src of commSources) {
    ...
    for (const type of ['commands', 'agents', 'rules']) {
      for (const item of src[type] || []) {
        ...
        // 去重：保留評級更高的版本
        if (itemsMap[key] && itemsMap[key].rating >= rating)
          continue
  ...
  if (!isEmpty(commSources)) {
    ...
    for (const src of commSources) {
      ...
      if (src.commands.length)
        parts.push(`${src.commands.length} 指令`)
      if (src.agents.length)
        parts.push(`${src.agents.length} 代理`)
      if (src.rules.length)
        parts.push(`${src.rules.length} 規則`)
      if (src.skills.length)
        parts.push(`${src.skills.length} 技能`)
      if (parts.length)
...
export function formatClaudeMd(plan) {
  ...
  if (!isEmpty(plan.projects)) {
    ...
    const parts = []
    if (mainPrj.length)
      parts.push(`${mainPrj.length} AI 生成`)
    if (tempPrj.length)
      parts.push(`${tempPrj.length} 靜態模板`)
    lines.push(
...
export function formatChanges(plan, existing) {
  ...
  if (newCmds.length)
    changes.push(`+${newCmds.length} cmd`)
  if (newAgents.length)
    changes.push(`+${newAgents.length} agent`)
  if (newRules.length)
    changes.push(`+${newRules.length} rule`)
  if (!existing.hasSettings)
