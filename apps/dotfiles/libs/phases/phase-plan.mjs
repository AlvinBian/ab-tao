...
export async function phasePlan(plan) {
  ...
  let finalPlan
  if (action === 'minimal')
    finalPlan = generateMinimalPlan(plan)
  else if (action === 'detail')
    finalPlan = await detailConfirm(plan)
  else finalPlan = plan
...
async function detailConfirm(originalPlan) {
  ...
  // 確保 plan 結構完整
  plan.repos ||= []
  plan.global ||= { commands: [], agents: [], rules: [], skills: [] }
  plan.global.commands ||= []
  plan.global.agents ||= []
  plan.global.rules ||= []
  plan.global.skills ||= []
