 ...
 * @param {object} _devDeps - devDependencies
  ...
  for (let i = 0; i < filtered.length; i += NPMS_BATCH_SIZE) {
    ...
    try {
      ...
      for (const [name, pkg] of Object.entries(data)) {
        ...
        if (popularity < NPMS_POPULARITY_THRESHOLD)
          continue
        ...
        if (priority > 45)
          continue // 排除 library(55), cli(50), devtool(60)
...
export async function analyzePhpDeps(composerDeps) {
  ...
  for (const r of starsResults) {
    ...
    if (stars < GITHUB_STARS_THRESHOLD)
      continue
