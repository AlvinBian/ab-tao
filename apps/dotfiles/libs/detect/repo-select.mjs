...
export async function interactiveRepoSelect(session = null) {
        ...
        return reposRaw
          ...
          .map((line) => {
            ...
            return {
              ...
              issues: Number.parseInt(issues, 10) || 0,
  ...
  function repoOpt(r, showOrg) {
    ...
    if (r.pct > 0)
      parts.push(`${r.pct}%`)
    if (r.commits > 0)
      parts.push(`${r.commits} commits`)
    if (r.stars > 0)
      parts.push(`★${r.stars}`)
    if (r.desc)
      parts.push(r.desc.slice(0, 30))
  ...
  if (multiOrg) {
    ...
    if (!isEmpty(preselected))
      p.log.info(showSummary(preselected))
