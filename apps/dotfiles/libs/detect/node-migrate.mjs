...
function discoverNvmDefault() {
  ...
  try {
    ...
    if (output !== 'N/A' && output.startsWith('v'))
...
function collectNDirs() {
  const dirs = new Set()
  if (existsSync(`${HOME}/.n`))
    dirs.add(`${HOME}/.n`)
  const nPrefix = process.env.N_PREFIX
  if (nPrefix && existsSync(`${nPrefix}/n`))
    dirs.add(`${nPrefix}/n`)
  if (existsSync('/usr/local/n'))
    dirs.add('/usr/local/n')
  return [...dirs]
}
