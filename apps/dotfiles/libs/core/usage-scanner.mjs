...
import { parseSourceFromContent } from './source-classifier.mjs'
        ...
        for (const text of texts) {
          ...
          if (agentMatches) {
            for (const m of agentMatches) {
              ...
              if (agentFilter && !agentFilter.has(name))
                continue
...
export async function collectFullStatus() {
  ...
  const classify = (name, allManaged, extList) => {
    if (allManaged.includes(name))
      return 'core'
    if (extList.includes(name))
      return 'ext'
    return 'user'
  }
  ...
  // ~/.zshrc loader marker 重複計數（> 1 表示有雙份注入）
  let zshrcLoaderCount = 0
  try {
    const { readFileSync } = await import('node:fs')
    const zshrcContent = readFileSync(path.join(HOME, '.zshrc'), 'utf8')
...
export function scanSkills(skillsDir) {
   ...
   *
   * 判斷優先順序：
   *   1. HTML comment 標記「generated by ab-tao[::source]」（ab-tao 安裝時寫入）
   *      - 有 source 子串：回傳 source（如 "ecc"、"anthropic"）
   *      - 無 source 子串：回傳 "ab-tao"
   *   2. YAML frontmatter 的 `origin:` 欄位（ECC 等外部 skill 通常有此欄）
   *   3. 無任何標記 → 回傳 "custom"（使用者自行安裝）
   *
   * 只讀取檔案前 400 字元，避免掃描大型 skill 檔案造成效能問題。
   *
   * @param {string} filePath - SKILL.md 或 SKILL.md.disabled 的絕對路徑
   * @returns {string} 來源標識符（"ecc" | "ab-tao" | "custom" | 其他小寫字串）
   */
  function parseSource(filePath) {
    try {
      // 只讀取檔案開頭 1024 bytes，委派給 source-classifier 統一判斷
 ...
 *
 * 資料來源分三層：
 *   1. live（即時）— collectFullStatus() + scanSkills()：每次呼叫重新掃描 ~/.claude/
...
export async function collectUnifiedReportData() {
  // 第一層：即時掃描（commands、agents、rules、hooks、zsh、plugins 等）
  const live = await collectFullStatus()

  // 即時掃描 skills 目錄（不依賴快取，確保顯示最新狀態）
  const skillsDir = path.join(HOME, '.claude', 'skills')
  live.skills = scanSkills(skillsDir)

  // 第二層：讀取 d:setup 寫入的快取（repos 物件與技術棧分類）
  ...
  try {
    ...
    /* 無快取（首次使用或快取損壞）時略過，使用空物件預設值 */
  }

  // 第三層：擴充資料（Wave 3 C2 新增的 5 個 Tab 資料來源）
  let extended = {}
  try {
    const { collectExtendedData } = await import(
      '../report/collect-unified.mjs',
    )
    extended = collectExtendedData()
  }
  catch {
    /* collect-unified.mjs 不存在或呼叫失敗時略過，extended 保持空物件 */
  }

  return {
    ...live,
    // 快取欄位：repos 強制物件化（防舊快取或上游殘留字串），供前端 ReposView 使用
    cachedRepos: (cached.repos || []).map(normalizeCachedRepo).filter(Boolean),
    // techStacks 相容處理：舊快取可能是扁平陣列，統一包裝成 { uncategorized: [...] }
    // 讓 renderTabTechStacks() 的 shouldClassify 邏輯可統一處理
    cachedTechStacks: Array.isArray(cached.techStacks)
      ? cached.techStacks.length > 0
        ? { uncategorized: cached.techStacks }
        : {}
      : cached.techStacks || {},
    // 快取產生時間戳，用於 Dashboard 顯示「上次 d:setup 時間」
...
/**
 * 正規化快取中的 repo：字串包成物件，物件原樣返回，其他丟棄
 * 為前端 Object.entries() 與 v-for 提供穩定結構，防 phase-complete.mjs 的 r.fullName 把物件變字串後，setup.mjs 又原樣寫入
 */
function normalizeCachedRepo(repo) {
  if (typeof repo === 'string')
    return { name: repo, role: 'unknown', localPath: repo }
  ...
  return new Promise((resolve) => {
    ...
    execFile(
      ...
      (err, stdout) => {
        if (err)
          return resolve(0)
        resolve(Number.parseInt(stdout.trim().split('\t')[0], 10) * 1024)
      },
    )
  })
}
