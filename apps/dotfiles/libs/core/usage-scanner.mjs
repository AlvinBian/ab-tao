...
export function scanSkills(skillsDir) {
	...
	function parseSource(filePath) {
		try {
			// 只讀取檔案開頭 400 字元，足以涵蓋 frontmatter 和 HTML comment
			const head = fs.readFileSync(filePath, "utf8").slice(0, 400);
			// 優先：HTML comment marker（ab-tao 安裝標記）
			const m = head.match(/<!--\s*generated by ab-tao(?:::([^\s>]+))?\s*-->/);
			if (m) return m[1] || "ab-tao";
			// 次要：YAML frontmatter origin 欄位（如 ECC skills）
			const originMatch = head.match(/^origin:\s*(\S+)/m);
			if (originMatch) return originMatch[1].toLowerCase();
		} catch {
			/* 讀取失敗則略過，回傳預設值 */
		}
		// 無任何來源標記：使用者自行安裝的 skill
		return "custom";
	}
...
export async function collectUnifiedReportData() {
	...
	return {
		...live,
		// 快取欄位：repos 保留完整物件格式（含 role/localPath），供 renderTabRepos() 使用
