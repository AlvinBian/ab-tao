...
function parseFrontmatter(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    // eslint-disable-next-line regexp/no-super-linear-backtracking -- YAML key-value parsing with bounded line input
...
function lintSkill(name, skillPath) {
  ...
  if (!fields.description) {
    ...
    if (len < 50)
      errors.push(`E2: description 太短（${len} 字元，需 ≥50）`)
    else if (len > 200)
  ...
  // E3 version
  if (!fields.version)
    errors.push('E3: 缺少 version（請加 version: 1.0.0）')
  ...
  if (lines > 500)
    warnings.push(`W2: 內容偏長（${lines} 行，建議 ≤500）`)
