...
async function syncSource(sourceName, config, options = {}) {
  ...
  try {
    ...
    // includePaths（allowlist）：只保留根層級列出的路徑，其餘全刪
    // 適用於結構明確的 source（如 anthropic 只需要 skills/）
    if (config.includePaths?.length > 0) {
      const includeSet = new Set(config.includePaths)
      for (const entry of fs.readdirSync(tempDir)) {
        if (!includeSet.has(entry)) {
          fs.rmSync(path.join(tempDir, entry), { recursive: true, force: true })
        }
      }
      console.warn(`  ✂️  只保留: ${config.includePaths.join(', ')}`)
    }

    // excludePaths（blocklist）：刪除已知的非必要路徑
    // 適用於「要的東西多、不要的東西少」的 source（如 gstack 60+ skill 目錄）
    for (const excludePath of (config.excludePaths ?? [])) {
      const fullPath = path.join(tempDir, excludePath)
      if (fs.existsSync(fullPath)) {
        fs.rmSync(fullPath, { recursive: true, force: true })
        console.warn(`  ⚠️  已排除: ${excludePath}`)
      }
    }
    ...
    try {
      ...
      // 用非同步 cp + filter 跳過 broken symlink（上游 repo 可能含指向已刪除 target 的 symlink）
      await fs.promises.cp(tempDir, targetPath, {
        recursive: true,
        dereference: true,
        filter: (src) => {
          try {
            ...
            if (stat.isSymbolicLink()) {
              fs.statSync(src) // 跟隨 symlink；broken 時丟 ENOENT
            }
            return true
          }
          catch {
            console.warn(
              `⚠️  跳過 broken symlink: ${path.relative(tempDir, src)}`,
            )
            return false
          }
        },
...
function listSources() {
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    if (stats) {
      ...
      if (stats.agents)
        parts.push(`${stats.agents} agents`)
      if (stats.rules)
        parts.push(`${stats.rules} rules`)
      if (stats.skills)
        parts.push(`${stats.skills} skills`)
