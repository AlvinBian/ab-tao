...
describe("validateFileContent", () => {
	it("安全檔案應通過驗證", () => {
		const content = fs.readFileSync(
			path.join(FIXTURES, "safe-file.md"),
			"utf8",
		);
	...
	it("markdown 中的 eval() 應產生警告而非攔截", () => {
		const content = fs.readFileSync(
			path.join(FIXTURES, "malicious-eval.md"),
			"utf8",
		);
	...
	it("markdown 中的 rm -rf 應產生警告而非攔截", () => {
		const content = fs.readFileSync(
			path.join(FIXTURES, "malicious-rm.md"),
			"utf8",
		);
	...
	it("任何檔案中的隱藏 HTML 指令應被偵測", () => {
		...
		assert.ok(
			result.warnings.some((w) => w.message.includes("hidden HTML directive")),
		);
	});

	// strict 模式 — 對文件檔也強制產生錯誤
	it("strict 模式下 markdown 中的 pattern 應被攔截", () => {
		const result = validateFileContent("test.md", 'eval("x")', {
			strict: true,
		});
		assert.equal(result.valid, false);
		assert.ok(result.errors.some((e) => e.code === "DANGEROUS_PATTERN"));
